                    ...
                    {rootNeurons.map((node, nodeIndex) => {
                      ...
                      return (
                        <div key={node.id} style={{
                          ...
                          <div style={{ position: 'absolute', inset: 0, borderRadius: BLOB, animation: `blobMorph ${node.isCore ? 12 : 20}s -${nodeIndex * 3}s infinite ease-in-out`, boxShadow: node.isCore ? `0 0 100px 30px rgba(${col},0.4)${(isActive || mapCfg.type === 'cluster') ? `, 0 0 160px 60px rgba(${col},0.5)` : ''}` : `0 0 20px 6px rgba(${col},0.12)` }} />
                          <div style={{ position: 'absolute', inset: 0, borderRadius: BLOB, animation: `blobMorph ${node.isCore ? 12 : 20}s -${nodeIndex * 3}s infinite ease-in-out`, overflow: 'hidden', background: node.isCore ? `radial-gradient(ellipse at 50% 50%, rgba(0,0,0,0) 40%, rgba(0,0,0,0.6) 70%, rgba(0,0,0,0.92) 100%), radial-gradient(ellipse at 40% 38%, rgba(${fill},0.55) 0%, rgba(${fill},0.3) 50%, rgba(${fill},0.08) 100%), rgb(4,4,8)` : `radial-gradient(ellipse at 50% 50%, rgba(0,0,0,0) 40%, rgba(0,0,0,0.72) 70%, rgba(0,0,0,0.92) 100%), radial-gradient(ellipse at 40% 38%, rgba(${fill},0.40) 0%, rgba(${fill},0.20) 50%, rgba(${fill},0.05) 100%), rgb(4,4,8)`, boxShadow: node.isCore ? `0 0 0 ${mapCfg.type === 'cluster' ? '9px' : '4.5px'} rgba(${col},${(isActive || mapCfg.type === 'cluster') ? '0.9' : '0.3'})` : `0 0 0 ${mapCfg.type === 'cluster' ? '9px' : '4.5px'} rgba(${col},0.2)` }}>
